...
class TransformerMonotonicEncoderLayer(TransformerEncoderLayer):
    def forward(self, x, encoder_padding_mask):
        if encoder_padding_mask is not None:
            # print("[Debug] mask= ", encoder_padding_mask[:, 0].shape)
...
class TransformerMonotonicDecoderLayer(TransformerDecoderLayer):
    ...
    def forward(
        ...
        if (torch.isnan(x).any()):
            raise KeyboardInterrupt("layer norm")
        ...
        ## self.cross_self-attention -> false
        ...
        if (torch.isnan(x).any()):
            raise KeyboardInterrupt("before self attn")
        ...
        x = self.dropout_module(x)
        x = self.residual_connection(x, residual)
        print("Debug onnx:", self.onnx_trace)
        # raise KeyboardInterrupt()
        if not self.normalize_before:
            x = self.final_layer_norm(x)
