...
class MonotonicAttention(MultiheadAttention):
    ...
    def __init__(self, args):
        ...
        print("Debug energy bias:", args.energy_bias, "at /gscratch/intelligentsystems/randy/fairseq/examples/simultaneous_translation/modules/monotonic_multihead_attention.py")
    ...
    def energy_from_qk(
        ...
        """
        print("Debug q, k:",query.shape, key.shape)
    ...
    def p_choose(self, query, key, key_padding_mask, incremental_states=None):
        print("p choose: Debug query", query.shape)
        print("p choose: Debug key", key.shape)
    ...
    def monotonic_attention_process_infer(
        ...
        if self.soft_attention:
            ...
            device = monotonic_step.device
            beta_mask = torch.arange(src_len, device=device).expand_as(alpha).gt(monotonic_step).unsqueeze(1)
    ...
    def forward(
        ...
        print("Debug line 353 query:", query.shape)
        
        src_len = value.size(0)

        if key_padding_mask is not None:
            # print(key_padding_mask[:,0], not key_padding_mask[:, 0].any(), key_padding_mask.shape)
            ...
            # print("Debug key padding mask:",key_padding_mask.shape)
            # raise KeyboardInterrupt()
